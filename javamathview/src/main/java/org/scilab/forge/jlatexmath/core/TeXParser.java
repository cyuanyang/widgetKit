...
public class TeXParser {
	...
	private Atom getScripts(char f) throws ParseException {
		...
		if (formula.root instanceof RowAtom) {
			...
			at = new PhantomAtom(new CharAtom('M', "mathnormal"), false, true,
					true);
		} else {
			at = formula.root;
			formula.root = null;
		...
		if (((c < '0' || c > '9') && (c < 'a' || c > 'z') && (c < 'A' || c > 'Z'))) {
			...
			if (symbolName == null
				...
				if (fontInfos != null) {
					...
					return new JavaFontRenderingAtom(parseString.substring(
				...
				if (!isPartial) {
					...
					if (AjLatexMath.IS_SHOW_ERROR_MSG){
	...
	private Atom processEscape() throws ParseException {
		...
		if (!isPartial) {
			...
			return new ColorAtom(new RomanAtom(new TeXFormula("\\backslash "
