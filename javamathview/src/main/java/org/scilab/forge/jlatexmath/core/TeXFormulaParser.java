...
/**
 * Parses a "TeXFormula"-element representing a predefined TeXFormula's from an
 * XML-file.
 */
public class TeXFormulaParser {
	...
	private class CreateTeXFormulaParser implements ActionParser {
		...
		public void parse(Element el) throws ResourceParseException {
			...
			// String code =
			// "TeXFormula.predefinedTeXFormulasAsString.put(\"%s\", \"%s\");";
			// System.out.println(String.format(code, formulaName,
			// argValues[0]));
			try {
				TeXFormula f = TeXFormula.class.getConstructor(argClasses)
						.newInstance(argValues);
				// succesfully created, so add to
				// "temporary formula's"-hashtable
				tempFormulas.put(name, f);
			} catch (Exception e) {
				throw new XMLResourceParseException(
						"Error creating the temporary TeXFormula '"
								+ name
								+ "' while constructing the predefined TeXFormula '"
								+ formulaName + "'!\n" + e.toString());
			}
	...
	private class CreateCommandParser implements ActionParser {
		...
		public void parse(Element el) throws ResourceParseException {
			...
			// create TeXFormula object
			try {
				MacroInfo f = MacroInfo.class.getConstructor(argClasses)
						.newInstance(argValues);
				// succesfully created, so add to
				// "temporary formula's"-hashtable
				...
				for (Class cl : argClasses) {
					err += "Created class: " + cl
							+ " loaded with the ClassLoader: "
							+ cl.getClassLoader() + "\n";
				}
				...
				throw new XMLResourceParseException(
						"Error creating the temporary command '"
								+ name
	...
	private class ReturnParser implements ActionParser {
		...
		public void parse(Element el) throws ResourceParseException {
			// get required string attribute
			String name = getAttrValueAndCheckIfNotNull("name", el);
			Object res = type == COMMAND ? tempCommands.get(name)
					: tempFormulas.get(name);
			if (res == null) {
				throw new XMLResourceParseException(
						PredefinedTeXFormulaParser.RESOURCE_NAME, RETURN_EL,
						"name",
	...
	private class TeXConstantsValueParser implements ArgumentValueParser {
		...
		public Object parseValue(String value, String type)
			...
			try {
				...
				int constant = TeXConstants.class.getDeclaredField(value)
						.getInt(null);
				...
				throw new XMLResourceParseException(
						PredefinedTeXFormulaParser.RESOURCE_NAME, "Argument",
						ARG_VAL_ATTR,
						"has an unknown constant name as value : '" + value
								+ "'!", e);
			}
		}
	}
		...
		public Object parseValue(String value, String type)
			...
			try {
				...
				throw new XMLResourceParseException(
						PredefinedTeXFormulaParser.RESOURCE_NAME, "Argument",
						ARG_VAL_ATTR,
						"has an unknown color constant name as value : '"
								+ value + "'!", e);
			}
		}
	}
	...
	private Object[] getArgumentValues(NodeList args) {
		...
		for (int j = 0; j < args.getLength(); j++) {
			...
			// parse value, hashtable will certainly contain a parser for the
			// class type,
	...
	private static void checkNullValue(String value, String type)
		...
		if (value.equals("")) {
			throw new XMLResourceParseException(
					PredefinedTeXFormulaParser.RESOURCE_NAME, "Argument",
					ARG_VAL_ATTR, "is required for an argument of type '"
							+ type + "'!");
		}
	}
